...
Эта версия использует только те поля, которые точно существуют в моделях.
"""
import os
import time
import json
...
def create_horoscope_block(system_key, content):
    ...
    try:
        ...
        # Create a dictionary for BlogBlock initialization with minimal fields that should exist
        blog_data = {
        ...
        # Add created_at if it exists in the columns
        if 'created_at' in blog_block_columns:
            blog_data['created_at'] = datetime.now()
        
        # Add position if it exists
        if 'position' in blog_block_columns:
            blog_data['position'] = get_position_for_system(system_key)
            
        # Add Russian fields if they exist in the model
        if 'title_ru' in blog_block_columns:
            blog_data['title_ru'] = f"{title_prefix}{system_name_ru} на {today.strftime('%d.%m.%Y')}"
        
        if 'content_ru' in blog_block_columns:
            blog_data['content_ru'] = content
            
        if 'summary_ru' in blog_block_columns:
            blog_data['summary_ru'] = get_blog_block_summary(content)
        
        # Ukrainian fields are added through direct SQL since they don't appear to be in the model
        ...
        # Now we need to update Ukrainian fields directly with SQL if needed
        # We know the ID of the new blog block, so we can do a direct update
        try:
            if all(col in blog_block_columns for col in ['title_uk', 'content_uk', 'summary_uk']):
                # Translate content to Ukrainian if the database supports it
                ...
                uk_title = f"{title_prefix}{system_name_uk} на {today.strftime('%d.%m.%Y')}"
                
                # Use SQLAlchemy core to update the Ukrainian fields directly
                from sqlalchemy import text
                
                sql = text("""
                    UPDATE blog_block 
                    SET title_uk = :title_uk, content_uk = :content_uk, summary_uk = :summary_uk 
                    WHERE id = :id
                """)
                ...
                db.session.execute(sql, {
                    ...
                    'summary_uk': uk_summary,
                    'id': blog_block.id
                })
                
                db.session.commit()
                logger.info(f"Added Ukrainian fields for {system_key} horoscope via SQL")
        except Exception as e:
            logger.error(f"Error updating Ukrainian fields: {e}")
...
def log_generation_activity(system, success, message=""):
    ...
    """
    try:
        # Create a dictionary for ContentGenerationLog initialization with minimal fields
        log_data = {
        ...
        if 'action' in content_log_columns:
            log_data['action'] = f"generate_{system}_horoscope"
            
        # Add created_at if available
        if 'created_at' in content_log_columns:
            log_data['created_at'] = datetime.now()
            
        # Add topic_id if required (with None value)
        if 'topic_id' in content_log_columns:
            log_data['topic_id'] = None
        
        # Create the log entry with the available fields
        log = ContentGenerationLog(**log_data)
        db.session.add(log)
        db.session.commit()
        
        # If system field exists in database but not model, add it via SQL
        try:
            if 'system' in content_log_columns:
                from sqlalchemy import text
                
                sql = text("""
                    UPDATE content_generation_log 
                    SET system = :system 
                    WHERE id = :id
                """)
            ...
            logger.error(f"Error updating system field: {e}")
            # This is not critical, so we can continue
