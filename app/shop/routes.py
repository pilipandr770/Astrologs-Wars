...
from app.models import Product, Category, Cart, CartItem, Order, OrderItem, User, Payment
from app import db
...
@shop.route('/shop')
def index():
    """Главная страница магазина с персональными прогнозами"""
...
def product(slug):
    """Страница продукта (персонального прогноза)"""
...
def checkout():
    ...
    if request.method == 'POST':
        # Обработка заказа
        payment_type = 'fiat'  # Теперь используем только стандартную оплату
